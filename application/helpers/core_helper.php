<?php
	...
	function is_logged_in() {
	    ...
	    if (isset($user['logged_in'])) { return true; } else { return false; }
	}

	function valid_token() {
	    $CI =& get_instance();
	    $user = $CI->session->userdata();
	    if ($user['valid_token'] == TRUE) { return true; } else { return false; }
	}
	...
	function sendsms($data) {
        ...
        return $CI->nexmo->get_http_status();
	}
	...
	function send_email($data){
				...
				$config = Array(
				'protocol' => 'smtp',
				'smtp_host' => 'smtp.api.createsend.com',
				'smtp_port' => 587,
				'smtp_user' => '64c0e028-048a-4cd7-a78e-fbe6c8522c8a',
				'smtp_pass' => '64c0e028-048a-4cd7-a78e-fbe6c8522c8a',
				'charset' => 'utf-8',
				'wordwrap' => true,
				'priority' => 1,
				'newline' => "\r\n",
	...
	function select($data) {
		$ordinal = array('Primero','Segundo','Tercero','Cuarto','Quinto','Sexto'); 
	    $CI =& get_instance();
	    $select = "<select id='{$data['id']}' class='{$data['clases']}' name='{$data['name']}'  {$data['multiple']}  >";
	    if($data['tipo'] == 'normal'){
		    $items = json_decode($data['items'],TRUE);
		    if($data['name']=='carrera')
		    	$select .= "<option value=''>No Aplica</option>";
		    foreach ($items as $key => $value) {
		    	$select .= "<option value='{$key}'>{$value}</option>";
		    }
		}elseif($data['tipo'] == 'ordinales'){
			$n = 1;
		    for ($i = 0; $i < $data['limite']; $i++) {
			    $select .= "<option value='".$n."'>".$ordinal[$i]."</option>";
			    $n++;
			}
		}elseif($data['tipo'] == 'abecedario'){
			$l = 65+$data['limite'];
			for($i=65; $i<=$l; $i++) {  
			    $letra = chr($i);  
			    $select .= "<option value='{$letra}'>{$letra}</option>";
			} 
		}
	    $select .= '</select>';
	  	return $select;
	...
	function jornada($n) {
		...
		$session = $ci->session->userdata('jornadas');
		$session =	json_decode($session[0]->valor,TRUE);
		return 	$session[$n];
	}
	...
	function grado($g,$n,$c){
      ...
      $ci->load->library('session');
		if($c==0){
			$session = $ci->session->userdata('niveles');
			$ret 	=	json_decode($session[0]->valor,TRUE);
			$dat 	= 	$ret[$n];
		}else{
			$session = $ci->session->userdata('carreras');
			$ret 	=	json_decode($session[0]->valor,TRUE);
			$dat 	= 	$ret[$c];
		}
